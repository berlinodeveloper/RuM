...
public class ExclusiveChoice implements TemplateReplayer {
	...
	public ExclusiveChoice (Map<DeclareTemplate, List<List<String>>> declareTemplateCandidateDispositionsMap){
		...
		for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Exclusive_Choice)){
		...
		if(!isEmpty){
			...
			if(isLastEvent){
				for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Exclusive_Choice)) {
					...
					if((seen.contains(param1)||seen.contains(param2)) && !(seen.contains(param1)&&seen.contains(param2))){
...
//			}	
	...
	public void updateModel(DeclareModel d, int completedTraces) {
		for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Exclusive_Choice)) {
			...
			d.addExclusiveChoices(param1, param2, completedTraces, satisfiedTraces.get(param1).get(param2), vacuouslySatisfiedTraces, violatedTraces.get(param1).get(param2));
		}
	}
