...
import org.processmining.plugins.correlation.ExtensionConfiguration;
...
@Plugin(name = "Declare Miner", parameterLabels = { "Log", "Declare Model", "Correlations" }, returnLabels = {
		"Mined Model" }, returnTypes = { DeclareMinerOutput.class }, userAccessible = true)
public class DeclareMiner {

	static boolean veloce = true;

	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "F.M. Maggi", email = "F.M.Maggi@tue.nl", uiLabel = "Declare Maps Miner", pack = "DeclareMiner")
	...
	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "F.M. Maggi", email = "F.M.Maggi@tue.nl", uiLabel = "Enhance a Declare Map with Correlations", pack = "DeclareMiner")
	@PluginVariant(requiredParameterLabels = { 0, 1, 2 }, variantLabel = "Enhance a Declare Map with Correlations")
	public DeclareMinerOutput analyse(UIPluginContext context, XLog log, DeclareMinerOutput inputModel,
			Correlations correlations) {
		ExtensionConfiguration conf = new ExtensionConfiguration();
		JComponent configuration = conf.showLogVis(correlations);
		// JScrollPane scrPane = new JScrollPane(configuration);
		// context.showWizard("Select Correlations", true, true, configuration);
		// return extend(context, log, inputModel, );
		return mineDeclareConstraints(context, log, inputModel, conf.getCorrelationsPerConstraint(correlations));
	}

	public static DeclareMinerOutput mineDeclareConstraints(UIPluginContext context, XLog log, DeclareMinerOutput model,
			Map<Integer, String> correlationsPerConstraint) {
		float support = model.getMinSupport();
		// float alpha = model.geta;
		// f = new FindItemSets(log, input);

		// Watch pruneLocalWatch = new Watch();
		// pruneLocalWatch.start();

		DeclareMinerInput input = model.getInput();
		// input.setAlpha(1);
		// Set<AprioriKnowledgeBasedCriteria> aprioriKnowledgeBasedCriteria = new
		// HashSet<AprioriKnowledgeBasedCriteria>();
		// input.setReferenceEventType("complete");
		// Map<DeclareTemplate, ConstraintTemplate> declareTemplateConstraintTemplateMap
		// = new HashMap<DeclareTemplate, ConstraintTemplate>();
		// Set<DeclareTemplate> selectedDeclareTemplateSet = new
		// HashSet<DeclareTemplate>();
		// input.setEventTypesMismatchLogModel(false);
		// input.setMinSupport((int)model.getMinSupport());
		// input.setMinSupport(model.getAlpha());

		// Map<String, DeclareTemplate> templateNameStringDeclareTemplateMap = new
		// HashMap<String, DeclareTemplate>();
		...
		// boolean eventTypesInModel = true;
		...
		// if(log.iterator().next().iterator().next().getAttributes().get(XLifecycleExtension.KEY_TRANSITION)!=null
		// && eventTypesInModel){
		// input.setEventTypesMismatchLogModel(false);
		// aprioriKnowledgeBasedCriteria.add(AprioriKnowledgeBasedCriteria.AllActivitiesWithEventTypes);
		// input.setAprioriKnowledgeBasedCriteriaSet(aprioriKnowledgeBasedCriteria);
		// }else{
		// input.setEventTypesMismatchLogModel(true);
		// aprioriKnowledgeBasedCriteria.add(AprioriKnowledgeBasedCriteria.AllActivitiesIgnoringEventTypes);
		// input.setAprioriKnowledgeBasedCriteriaSet(aprioriKnowledgeBasedCriteria);
		// }
		...
		HashMap<DeclareTemplate, Vector<MetricsValues>> metricsValuesPerTemplate = new HashMap<DeclareTemplate, Vector<MetricsValues>>();

		TemplateInfo templateInfo = null;

		for (ConstraintDefinition cd : model.getModel().getModel().getConstraintDefinitions()) {
			// startLogging(selectedDeclareTemplate, pruneLocalWatch);
			Vector<MetricsValues> metricsValues = new Vector<MetricsValues>();
			if (metricsValuesPerTemplate.containsKey(DeclareMiner.getTemplate(cd))) {
				metricsValues = metricsValuesPerTemplate.get(DeclareMiner.getTemplate(cd));
			}
			...
			switch (DeclareMiner.getTemplate(cd)) {
				...
				case Chain_Succession:
					...
					values = ((SuccessionInfo) templateInfo).getMetrics(model, cd,
							correlationsPerConstraint.get(cd.getId()), input, model.getAlpha(), support, log,
							DeclareMiner.getTemplate(cd), context);
					break;
				case Choice:
				...
				case Chain_Precedence:
					...
					values = ((PrecedenceInfo) templateInfo).getMetrics(model, cd,
							correlationsPerConstraint.get(cd.getId()), input, model.getAlpha(), support, log,
							DeclareMiner.getTemplate(cd), context);
				...
				case Chain_Response:
					...
					values = ((ResponseInfo) templateInfo).getMetrics(model, cd,
							correlationsPerConstraint.get(cd.getId()), input, model.getAlpha(), support, log,
							DeclareMiner.getTemplate(cd), context);
					break;
				case CoExistence:
					...
					values = ((CoexistenceInfo) templateInfo).getMetrics(model, cd,
							correlationsPerConstraint.get(cd.getId()), input, model.getAlpha(), support, log,
							DeclareMiner.getTemplate(cd), context);
			...
			metricsValuesPerTemplate.put(DeclareMiner.getTemplate(cd), metricsValues);
			// endLogging(selectedDeclareTemplate, pruneLocalWatch,metricsValues.size(),
			// templateInfo.getNumberOfDiscoveredConstraints());
		...
		DeclareModelGenerator dmg = new DeclareModelGenerator();
		DeclareMinerOutput output = dmg.createModel(true, true, metricsValuesPerTemplate, log, input,
	...
	public static DeclareMinerOutput mineDeclareConstraints(UIPluginContext context, XLog log, DeclareMinerInput input,
		...
		try {
			...
			if (!veloce) {
				w.start();
				declareMinerOutput = pruner.prune(true, true, activityNameList,
						frequentItemSetTypeFrequentItemSetSupportMap, declareTemplateCandidateDispositionsMap,
						aprioriSupportValues, printVerbose, input);
				System.out.println("il lento: " + w.msecs());
			} else {
				w.start();
				declareMinerOutput = pruner.fastPrune(true, true, log, input, declareTemplateCandidateDispositionsMap);
				System.out.println("il veloce: " + w.msecs());
			}
			int constraintsNo = 0;
	...
	public static DeclareMinerOutput mineDeclareConstraints(UIPluginContext context, XLog log, DeclareMinerInput input,
		...
		try {
			...
			// declareMinerOutput =
			// pruner.prune(frequentItemSetTypeFrequentItemSetSupportMap,
	...
	public static Map<DeclareTemplate, ConstraintTemplate> readConstraintTemplates(
		...
		InputStream templateInputStream = DeclareMiner.class.getResourceAsStream("/resources/template.xml");
