...
public class ChainPrecedence implements TemplateReplayer {
	...
	public ChainPrecedence (Map<DeclareTemplate, List<List<String>>> declareTemplateCandidateDispositionsMap){
		
		for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Chain_Precedence)){
				...
				for(String param2 : fulfilledForThisTrace.get(param1).keySet()) {
					if(!param1.equals(param2)){
						if(seenEvents.containsKey(param2)){
							if (fulfilledForThisTrace.get(param1).get(param2) == seenEvents.get(param2)) {
		...
		for(String param1 : fulfilledForThisTrace.keySet()) {
			for(String param2 : fulfilledForThisTrace.get(param1).keySet()) {
				if(!param1.equals(param2)){					
					...
					d.addChainPrecedence(param1, param2, completedTraces, satisfiedTraces.get(param1).get(param2), vacuouslySatisfied, violatedTraces.get(param1).get(param2));
				}
			}
		}
	}
