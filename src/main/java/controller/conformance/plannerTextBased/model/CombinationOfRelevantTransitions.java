...
public class CombinationOfRelevantTransitions {
	...
	public CombinationOfRelevantTransitions(String cotID, String comb_label, int length_of_the_combination, Vector<String> vector_with_combination_of_transitions, Vector<String> original_transitions_associated_to_the_label) {		
		id = cotID;
		label = comb_label;
	...
	private void setPreconditionsAndEffects() {
		...
		for(int m=0;m<transitions_in_the_combination_vector.size();m++) {
			...
			if (Constants.getAutomataSinkNonAcceptingStatesVector().contains(rt.getTarget_state())) {
		...
		for(int n=0;n<all_transitions_associated_to_the_label_vector.size();n++) {
			...
			if(!automata_vector.contains(automaton_id)) {
				...
				RelevantTransition rt2 = Utilities.getRelevantTransition(rt_not_in_the_combination_ID);
