...
public class Absence implements TemplateReplayer {
	...
	public Absence (Map<DeclareTemplate, List<List<String>>> declareTemplateCandidateDispositionsMap){
		for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Absence)){
		...
		if(!isEmpty){
			...
			if(isLastEvent ){
				for(String param1 : fulfilledForThisTrace.keySet()) {
					...
					if(fulfilledForThisTrace.get(param1)==0){
					...
					//	d.addAbsence(param1, completedTraces, satisfiedTraces, vacuouslySatisfiedTraces, violatedTraces);
				}
			}
		}else{
			for(String param1 : fulfilledForThisTrace.keySet()) {
	...
	public void updateModel(DeclareModel d, int completedTraces) {
		for(String param1 : fulfilledForThisTrace.keySet()) {
			...
			d.addAbsence(param1, completedTraces, satisfiedTraces.get(param1), vacuouslySatisfiedTraces, violatedTraces.get(param1));
