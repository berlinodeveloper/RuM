...
public class TemplateListPanel extends JPanel {
	...
	Map<String, DataDeclareTemplate> templateNameStringDataDeclareTemplateMap;
	...
	private void populateDataDeclareTemplates() {
		DataDeclareTemplate declareTemplate = DataDeclareTemplate.Alternate_Precedence;
		DataDeclareTemplate[] declareTemplateNames = declareTemplate.getDeclaringClass().getEnumConstants();

		templateNameStringDataDeclareTemplateMap = new HashMap<String, DataDeclareTemplate>();

		for (DataDeclareTemplate d : declareTemplateNames) {
			String templateNameString = d.toString().replaceAll("_", " ").toLowerCase();
			if (templateNameString.equals("not ch precedence")) {
				templateNameString = "not chain precedence";
			}
			if (templateNameString.equals("not ch response")) {
				templateNameString = "not chain response";
			}
			templateNameStringDataDeclareTemplateMap.put(templateNameString, d);
		}
	}
	...
	private void preparePanel() {
		...
		for (String templateNameString : templateNameStringDataDeclareTemplateMap.keySet()) {
		...
		templatesTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (selectedRows.length > 1) {
					...
					DataDeclareTemplate declareTemplate = templateNameStringDataDeclareTemplateMap
							.get(selectedTemplateString);
	...
	private void readConstraintTemplates() {
		...
		declareTemplateConstraintTemplateMap = new HashMap<DataDeclareTemplate, ConstraintTemplate>();
		...
		for (IItem item : templateList) {
			if (item instanceof ConstraintTemplate) {
				...
				if (templateNameStringDataDeclareTemplateMap
						.containsKey(constraintTemplate.getName().replaceAll("-", "").toLowerCase())) {
					declareTemplateConstraintTemplateMap.put(
							templateNameStringDataDeclareTemplateMap
									.get(constraintTemplate.getName().replaceAll("-", "").toLowerCase()),
							constraintTemplate);
					System.out.println(constraintTemplate.getName() + " @ " + templateNameStringDataDeclareTemplateMap
							.get(constraintTemplate.getName().replaceAll("-", "").toLowerCase()));
				}
			}
		}
	...
	public Set<String> getAllRows() {
		return templateNameStringDataDeclareTemplateMap.keySet();
	}

	public Set<DataDeclareTemplate> getAllDataDeclareTemplates() {
		Set<DataDeclareTemplate> selectedDataDeclareTemplateSet = new HashSet<DataDeclareTemplate>();
		Set<String> selectedTemplateNameSet = getAllRows();
		for (String selectedTemplateName : selectedTemplateNameSet)
			selectedDataDeclareTemplateSet.add(templateNameStringDataDeclareTemplateMap.get(selectedTemplateName));
	...
	public Set<DataDeclareTemplate> getSelectedDataDeclareTemplates() {
		Set<DataDeclareTemplate> selectedDataDeclareTemplateSet = new HashSet<DataDeclareTemplate>();
	...
	public Set<DataDeclareTemplate> getDataDeclareTemplates(Set<String> templateNameSet) {
		Set<DataDeclareTemplate> declareTemplateSet = new HashSet<DataDeclareTemplate>();
		for (String templateName : templateNameSet)
			declareTemplateSet.add(templateNameStringDataDeclareTemplateMap.get(templateName));
		return declareTemplateSet;
	}

	public Set<String> getTemplates(Set<DataDeclareTemplate> declareTemplateSet) {
		Set<String> templateNameSet = new HashSet<String>();
		for (DataDeclareTemplate declareTemplate : declareTemplateSet) {
	...
	public Map<DataDeclareTemplate, ConstraintTemplate> getDataDeclareTemplateConstraintTemplateMap() {
