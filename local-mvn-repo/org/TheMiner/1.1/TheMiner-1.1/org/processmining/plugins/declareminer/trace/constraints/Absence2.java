...
public class Absence2 implements TemplateReplayer {
	...
	public Absence2 (Map<DeclareTemplate, List<List<String>>> declareTemplateCandidateDispositionsMap){
		for(List<String> params : declareTemplateCandidateDispositionsMap.get(DeclareTemplate.Absence2)){
		...
		if(!isEmpty){
			...
			if(isLastEvent){
				for(String param1 : fulfilledForThisTrace.keySet()) {
					...
					//	d.addAbsence2(param1, completedTraces, satisfiedTraces, vacuouslySatisfiedTraces, violatedTraces);
				}
			}
		}else{
			for(String param1 : fulfilledForThisTrace.keySet()) {
	...
	public void updateModel(DeclareModel d, int completedTraces) {
		for(String param1 : fulfilledForThisTrace.keySet()) {
			...
			d.addAbsence2(param1, completedTraces, satisfiedTraces.get(param1), vacuouslySatisfiedTraces, violatedTraces.get(param1));
