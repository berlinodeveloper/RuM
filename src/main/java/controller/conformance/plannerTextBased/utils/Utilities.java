...
import controller.conformance.plannerTextBased.model.CombinationOfAcceptingStates;
...
public class Utilities {
  ...
  public static void findCombinationsOfTransitions(Object[] arr, String label, int len, int original_k_value, int startPosition, String[] result) {
    if (len == 0) {
      ...
      CombinationOfRelevantTransitions cot = new CombinationOfRelevantTransitions(cotID, label, original_k_value, combination_of_relevant_transitions_vector,
          original_transitions_associated_to_the_label_vector);
  ...
  public static StringBuffer createPropositionalDomain(Trace trace) {
    ...
    for (int i = 0; i < Constants.getCombinationOfRelevantTransitions_vector().size(); i++) {
      ...
      //
      if (trace.getTraceAlphabet_vector().contains(label_of_the_cot) || Constants.getAlphabetOfTheConstraintsVector().contains(label_of_the_cot)) {
        ...
        for (int yu = 0; yu < trace.getOriginalTraceContent_vector().size(); yu++) {
          Vector<String> specificTraceCostVector = new Vector<>();
          specificTraceCostVector.addElement(trace.getOriginalTraceContent_vector().elementAt(yu));
          specificTraceCostVector.addElement("1");
          specificTraceCostVector.addElement("1");
    ...
    for (int op = 0; op < Constants.getAutomata_abstract_accepting_states_vector().size(); op++) {
      String abstract_state_id = Constants.getAutomata_abstract_accepting_states_vector().elementAt(op);
